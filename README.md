...
Each timer is one naturally wrapping line: `YYYY-MM-DD HH:MM` • Discord live relative timestamp • System (LY • Range) • Structure • Timer • Objective. Dates are EVE time (UTC), in inline code for consistent formatting. Discord updates the relative time automatically without bot edits. Objectives are Friendly, Hostile, Neutral, or Undefined. Distance is from the main staging system (falling back to the first configured staging system), named above each message. Cached distances round up to one decimal place. Range tags use the unrounded distance: Super below 6 LY, Cap from 6 to below 7 LY, Command carrier from 7 to below 7.5 LY, and no tag at 7.5 LY or above. Missing distances display `? LY`. System names link to their Dotlan map using `/map/region/system-name`, with website previews suppressed. Location details are omitted. Standard racial control towers display as `POS`, `POS Small`, or `POS Medium`; no size suffix means large. Fortizar, Tatara, and Sotiyo names appear in bold; Keepstar appears in bold with a 🏰 beside it. Stored structure names are unchanged.

Upcoming timers appear next due first. Blank lines separate rolling 24-hour groups measured from refresh time; recently elapsed timers form a separate group at the bottom and disappear after one hour without being deleted from the app. Empty groups add no extra spacing. Messages split only between timers and stay within Discord’s 2,000-character limit. Text fields are bounded and Discord markup is escaped. The bot uses discordproxy to edit existing messages only when their content changes; there is no countdown footer.
